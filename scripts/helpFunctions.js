...
function findErrors(tree) {
    const errorNodes = [];
    collectErrorNodes(tree.rootNode, errorNodes);
    return errorNodes;
}
 ...
 * @param {MyNode} root_node The root node of the tree.
 ...
 */
function createBlocklyCode(root_node, distributor, bodySetter, outerBlock ){
    let currentBlock; // Variable to store temporary block
    ...
    for (const node of root_node.children) {
        ...
        const nodeObject = distributor.getNodeObject(node, lastBlock);
        if (nodeObject instanceof CommentNode && !isInner){
            // Ignore comments in the main body to avoid infinite loop of generating comments for functions
            wasComment = true;
            continue
        }
        currentBlock = nodeObject.createBlock();

        // Deal with connecting the blocks or moving the new block from last one
        if (isInner){
            if (outerBlock !== null && currentBlock !== null) {
              statementConnection = outerBlock.getInput(bodySetter).connection;
              statementConnection.connect(currentBlock.previousConnection);
               outerBlock = null;
            }
            else if (currentBlock !== null && lastBlock !== null) {
               currentBlock.previousConnection.connect(lastBlock.nextConnection);
            }
        }
            ...
            if (node.previousSibling.type === 'empty_line'
                ...
                if (currentBlock !== lastBlock) {
                    ...
                    currentBlock.moveBy(blockPosition.x + 0, blockPosition.y + lastBlock.height + 30);
                }
            }
            else if (lastBlock.type !== 'procedures_defnoreturn' || lastBlock.type !== 'procedures_defreturn')
                currentBlock.previousConnection.connect(lastBlock.nextConnection);
        }
        lastBlock = currentBlock; // Update last block
        wasComment = false;
    }
