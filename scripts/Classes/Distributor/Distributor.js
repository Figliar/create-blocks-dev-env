...
class Distributor {
     ...
     * @param {MyNode} node The node to get the subclass for.
     * @param {Blockly.Block} lastBlock The last block generated.
     * @returns {MyNode} The node object of the appropriate subclass.
     * @throws {Error} If no subclass is found for the given node type.
     */
    getNodeObject(node, lastBlock) {
        const Subclass = this.nodeToSubclassMap[node.type];
        if (Subclass) {
            return new Subclass(node, this, lastBlock);
        } else {
     ...
     * @param {MyNode} Subclass The subclass to register.
     */
    ...
    setFunctionsReturnLOVE(){
        return {
            ...
            "love.filesystem.write": "filesystem_write",
