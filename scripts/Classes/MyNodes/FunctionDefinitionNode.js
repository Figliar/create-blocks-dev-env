...
class FunctionDefinitionNode extends MyNode {
    ...
    _createKnownFunctionDefinitionBlock() {
        ...
        createBlocklyCode(includedComments, this.distributor, "input", functionBlock);
    ...
    _createUnknownFunctionDefinitionBlock() {
        ...
        if (functionBodyNode !== null) {
            let includedComments = this._includeComments(this.node);
            createBlocklyCode(includedComments, this.distributor, "STACK", functionBlock);
        }

        if (isReturnNode !== null) {
            if (isReturnNode.childCount > 1) {
                let returnValueNodeHandler = this.distributor.getNodeObject(isReturnNode.lastChild.firstChild, null);
    ...
    /**
     * Finds the function body node in the given node.
     ...
     * @param {MyNode|null} node The node to search for the return statement.
     * @returns {MyNode|null} The return statement node if found, otherwise null.
     * @private
     */
